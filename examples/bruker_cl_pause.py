...
def main():
    ...
    # Trial structure
    trial_labels = np.array(["inc_r_inv","inc_l_inv","con_r_inv","con_l_inv"]) # visible, consistent. 00, 01, 10, 11
    ...
    if save_history:
        ...
        if req_fix:
            params.update(
                ...
                'return_max_duration':return_max_duration}
            )
    ...
    if save_history:
        fs_manager.set_save_history_params(save_history_flag=save_history, save_path=save_path, fs_frame_rate_estimate=fs_frame_rate, save_duration=inc_stim_duration+return_max_duration+(fix_max_duration if req_fix else iti))
    ...
    for t in range(n_trials):
        ...
        if req_fix:
            ...
            if return_to_0 is not None:
                ...
                if return_to_0 == "OL":
                    ...
                    return_to_0_success = True
                    while abs(theta_diff) > 1 and return_to_0_success: #within 1 degree of bar facing forward
                        _,_,[theta_rad] = ft_manager.update_pos(update_theta=True)
                        ts = time()
                        ...
                        if time()-t_start > return_max_duration:
                            return_to_0_success = False

                fs_manager.stop_stim()
                t_end = time()
                ft_manager.set_pos_0(theta_0=None, x_0=0, y_0=0)
                theta_return_duration = t_end-t_start
                if return_to_0_success:
                    print(f"===== Returned {theta_return_deg:.2f}degs in {theta_return_duration:.{5}}s ======")
                else:
                    print(f"===== Return failed in {theta_return_duration:.{5}}s ======")
        ...
        if req_fix:
            ...
            if return_to_0 is not None:
                return_to_0_successes.append(return_to_0_success)
        ...
        for t in range(n_trials):
            ...
            if req_fix:
                ...
                if return_to_0 is not None:
                    ...
                    trial.attrs['return_to_0_success'] = return_to_0_successes[t]
